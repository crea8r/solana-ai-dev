...
export const chatAI = async (text: string, fileContext?: { path: string; content: string }) => {
  const body = { 
    messages: [{ content: text }],
    fileContext: fileContext ? { path: fileContext.path, content: fileContext.content } : null,
  };
